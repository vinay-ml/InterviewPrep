const javaScriptCodingQuestions = require("../models/javaScriptCodingQuestions");
const cloudinary = require("../config/cloudinaryConfig");
const { getCloudinaryPublicId } = require("../utils/cloudinaryUtils");

// ** Get All Questions **
exports.getAllJavaScriptCodingQuestions = async (req, res) => {
  try {
    const questions = await javaScriptCodingQuestions.find();
    res.status(200).json({
      message: "All Javascript questions retrieved successfully",
      data: questions,
    });
  } catch (error) {
    res.status(500).json({
...
// ** Add a New Question **
exports.addJavaScriptCodingQuestion = async (req, res) => {
  try {
    ...
    const newQuestion = new javaScriptCodingQuestions({
      title,
    ...
    res.status(201).json({
      message: "Javascript question added successfully",
      data: newQuestion,
    });
  } catch (error) {
    res.status(500).json({
...
exports.updateJavaScriptCodingQuestion = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, answers, sampleCode, videoURL } = req.body;

    const question = await javaScriptCodingQuestions.findById(id);
    if (!question) {
    ...
    await question.save();
    res.status(200).json({
      message: "JavaScript question updated successfully",
      data: question,
    });
  } catch (error) {
    res.status(500).json({
...
exports.deleteJavaScriptCodingQuestionImage = async (req, res) => {
  try {
...
exports.deleteJavaScriptCodingQuestion = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await javaScriptCodingQuestions.findByIdAndDelete(id);
