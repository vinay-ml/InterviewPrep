const HTMLQuestions = require("../models/HTMLQuestions");
const cloudinary = require("../config/cloudinaryConfig");
const { getCloudinaryPublicId } = require("../utils/cloudinaryUtils");

// ** Get All Questions **
exports.getAllHTMLQuestions = async (req, res) => {
  try {
    const questions = await HTMLQuestions.find();
    res.status(200).json({
      message: "All HTML questions retrieved successfully",
      data: questions,
    });
  } catch (error) {
    res.status(500).json({
...
// ** Add a New Question **
exports.addHTMLQuestion = async (req, res) => {
  try {
    ...
    const newQuestion = new HTMLQuestions({
      title,
...
exports.updateHTMLQuestion = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, answers, sampleCode, videoURL } = req.body;

    const question = await HTMLQuestions.findById(id);
    if (!question) {
    ...
    await question.save();
    res.status(200).json({
      message: "HTML question updated successfully",
      data: question,
    });
  } catch (error) {
    res.status(500).json({
...
exports.deleteHTMLQuestionImage = async (req, res) => {
  try {
...
exports.deleteHTMLQuestion = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await HTMLQuestions.findByIdAndDelete(id);
