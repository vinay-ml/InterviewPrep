const PowerBIQuestions = require("../models/PowerBIQuestions");
const cloudinary = require("../config/cloudinaryConfig");
const { getCloudinaryPublicId } = require("../utils/cloudinaryUtils");

// ** Get All Questions **
exports.getAllPowerBIQuestions = async (req, res) => {
  try {
    const questions = await PowerBIQuestions.find();
    res.status(200).json({
      message: "All PowerBI questions retrieved successfully",
      data: questions,
    });
  } catch (error) {
    res.status(500).json({
...
// ** Add a New Question **
exports.addPowerBIQuestion = async (req, res) => {
  try {
    ...
    const newQuestion = new PowerBIQuestions({
      title,
    ...
    res.status(201).json({
      message: "PowerBI question added successfully",
      data: newQuestion,
    });
  } catch (error) {
    res.status(500).json({
...
exports.updatePowerBIQuestion = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, answers, sampleCode, videoURL } = req.body;

    const question = await PowerBIQuestions.findById(id);
    if (!question) {
    ...
    await question.save();
    res.status(200).json({
      message: "PowerBI question updated successfully",
      data: question,
    });
  } catch (error) {
    res.status(500).json({
...
exports.deletePowerBIQuestionImage = async (req, res) => {
  try {
...
exports.deletePowerBIQuestion = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    await PowerBIQuestions.findByIdAndDelete(id);
